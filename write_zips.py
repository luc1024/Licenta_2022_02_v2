...
def write_zips(pdf_folder: str, zip_folder: str, dict_zile: dict, nr_comisii: int):
    ...
    for cod in itertools.product(list(dict_zile.keys()), [chr(i) for i in range_comisii]):
        f = '_' + ''.join(cod) + '_'
        fisiere_comisie_s = [ff for ff in os.listdir(pdf_folder + 'subiecte' + os.sep) if
                             ff.find(f) > -1 and ff[ff.find('.'):] == '.pdf']
        fisiere_comisie_r = [ff for ff in os.listdir(pdf_folder + 'raspunsuri' + os.sep) if
                             ff.find(f) > -1 and ff[ff.find('.'):] == '.pdf']
        if len(fisiere_comisie_s) > 0:
            ...
            for file_name in fisiere_comisie_s:
                ...
                zip_file.write(pdf_folder + 'subiecte' + os.sep + file_name, subf + os.sep + file_name,
                               compress_type=zipfile.ZIP_DEFLATED)
            for file_name in fisiere_comisie_r:
                subf = subfolder[(file_name.find('Subiect') == -1)]
                zip_file.write(pdf_folder + 'raspunsuri' + os.sep + file_name, subf + os.sep + file_name,
                               compress_type=zipfile.ZIP_DEFLATED)
